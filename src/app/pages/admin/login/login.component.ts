...
export class AdminLoginComponent implements OnInit {
    ...
    if (this.loginForm.valid) {
      this.authService.loginAdmin(email, password).subscribe({
        next: (result: any) => {
          if (result.user) {
            this.loading.hide();
            sessionStorage.setItem('userAdmin', JSON.stringify(result.user));
  ...
  notify2(titulo: string = "Aviso!", mensagem: string = "Código enviado para o e-mail cadastrado.", redirect: string = '/login-adm') {
